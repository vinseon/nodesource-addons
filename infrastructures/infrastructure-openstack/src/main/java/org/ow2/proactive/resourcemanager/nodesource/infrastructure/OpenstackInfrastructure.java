...
public class OpenstackInfrastructure extends InfrastructureManager {
	...
	@Configurable(description = "The The Openstack_Password")
	...
	public void acquireNode() {
		...
		for (int i = 1; i <= numberOfInstances; i++) {
			...
			String instanceJson = ConnectorIaasJSONTransformer.getInstanceJSON(instanceTag, image, "1", publicKeyName,
					String.valueOf(flavor), scripts);
	...
	private String generateDefaultStartNodeCommand(String instanceId) {
		try {
			...
			return "java -jar node.jar -D" + INSTANCE_TAG_NODE_PROPERTY + "=" + instanceId + " " + additionalProperties
